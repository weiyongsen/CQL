...
# 加载训练好的模型
checkpoint_path = r"save_model\2025-04-01_10-18-05\checkpoint_000951"  # 修改为您的checkpoint路径
algo.restore(checkpoint_path)
print(f"✅ 成功加载模型：{checkpoint_path}")

# 评估循环
episode_data = []  # 存储每个episode的数据
for episode in range(3):  # 评估3个episode
    obs = env.reset(h_initial=5000, h_setpoint=8000, 
                    psi_initial=90, psi_setpoint=200,
                    v_initial=500, v_setpoint=600)
    total_reward = 0
    done = False
...
fig.suptitle('Flight Control Performance Analysis', fontsize=14, y=0.95)
...
for i, data in enumerate(episode_data):
    ax1.plot(data['height'], color=colors[i], linewidth=1.5, label=f'Episode {i+1}')
    ax1.axhline(y=data['target_height'], color=target_color, linestyle='--', linewidth=1, label=f'Target (Ep {i+1})')
...
for i, data in enumerate(episode_data):
    ax2.plot(data['heading'], color=colors[i], linewidth=1.5, label=f'Episode {i+1}')
    ax2.axhline(y=data['target_heading'], color=target_color, linestyle='--', linewidth=1, label=f'Target (Ep {i+1})')
...
for i, data in enumerate(episode_data):
    ax3.plot(data['velocity'], color=colors[i], linewidth=1.5, label=f'Episode {i+1}')
    ax3.axhline(y=data['target_velocity'], color=target_color, linestyle='--', linewidth=1, label=f'Target (Ep {i+1})')
